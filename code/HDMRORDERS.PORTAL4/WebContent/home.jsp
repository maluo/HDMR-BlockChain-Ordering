...
<body>
	...
	<form action="${registerUrl}" method="post">
		<table>
			...
			<c:if test="${order.getId() ne null}">
				<tr>
					<td>ID:</td>
					<td><input type="text" name="id" value="${order.getId()}"
			...
			<tr>
				...
				<td><input type="text" name="order_num"
					value="${order.getOrderNum()}" required></td>
			</tr>

			<tr>
				<td>Import Number:</td>
				<td><input type="text" name="order_num_post"
					value="${order.getOrderNum2()}" required></td>
			</tr>

			<tr>
				<td>Item Number</td>
				<td><input type="text" name="item_num"
					value="${order.getItems().getProductId()}" readonly="readonly"></td>
			</tr>

			<tr>
				<td>Unit Price</td>
				<td><input type="text" name="unit_price"
					value="${order.getUnitPrice()}" required></td>
			</tr>

			<tr>
				<td>Quantity</td>
				<td><input type="text" name="quantity"
					value="${order.getQuantity()}" required></td>
			</tr>

			<tr>
				<td>Shipping</td>
				<td><input type="text" name="shipping"
					value="${order.getShipping()}" required></td>
			</tr>

			<tr>
				<td>Transaction Date</td>
				<td><input type="text" name="transaction_date"
					value="${order.getTransactionDate()}" required></td>
			</tr>

			<tr>
				<td>Sys Date</td>
				<td><input type="text" name="sys_date"
					value="${order.getSysDate()}" readonly="readonly"></td>
			</tr>
	...
	<form action="<c:url value="/order/dash"/>" method="post">
		<table>
			<tr>
				<td>Budget:</td>
				<td><input type="text" name="budget" value="0" /></td>
				<td colspan="2"><input type="submit" value="Plan Your Budget"></td>
			</tr>
			
			<c:if test="${budgetPlan ne null}">
			<tr>
			<pre style="margin:0px;">${budgetPlan}</pre>
			</tr>
			</c:if>
		</table>
	</form>
